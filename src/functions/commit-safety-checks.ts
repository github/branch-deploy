...
import {isTimestampOlder} from './is-timestamp-older.ts'
import {saveActionState, setActionOutput} from '../action-io.ts'
import {legacyIssueCommentCreatedAt, legacyTruthy} from '../trust-boundaries.ts'
import type {
  BranchDeployContext,
  CommitSafetyData,
  CommitSafetyResult
...
export function commitSafetyChecks(
  context: BranchDeployContext,
  data: CommitSafetyData
): CommitSafetyResult {
  ...
  core.debug(`comment_created_at: ${String(comment_created_at)}`)
  core.debug(`commit_created_at: ${String(commit_created_at)}`)
  core.debug(`verified_at: ${String(verified_at)}`)

  // Defensive: Ensure required fields exist
  if (!legacyTruthy(comment_created_at)) {
    throw new Error('Missing context.payload.comment.created_at')
  }
  if (!legacyTruthy(commit_created_at)) {
  ...
  } else if (inputs.commit_verification) {
  ...
  if (inputs.commit_verification && !legacyTruthy(verified_at)) {
    return {
