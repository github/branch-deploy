...
export async function postDeployComment(context, octokit, deployment_comment_id, deployment_status, deployment_message, deployment_result_ref, deployment_mode_noop) {

  // Check the inputs to ensure they are valid
  if (deployment_comment_id && deployment_status && deployment_message && deployment_result_ref && deployment_mode_noop) {
    core.info('post deploy comment logic triggered... executing')
  } else if (!deployment_comment_id || deployment_comment_id.length === 0) {
    core.info('No deployment_comment_id provided, skipping post-deployment comment logic')
    return false
  } else if (!deployment_status || deployment_status.length === 0) {
    throw new Error('deployment_comment_id specified but no deployment_status provided')
  } else if (!deployment_message || deployment_message.length === 0) {
    throw new Error('deployment_comment_id specified but no deployment_message provided')
  } else if (!deployment_result_ref || deployment_result_ref.length === 0) {
    throw new Error('deployment_comment_id specified but no deployment_result_ref provided')
  } else {
    core.info('An unhandled condition was encountered, skipping post-deployment comment logic')
    return false
  }
  ...
  actionStatus(context, octokit, parseInt(deployment_comment_id), deployment_message_fmt, success)
