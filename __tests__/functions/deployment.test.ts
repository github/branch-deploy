...
interface TestDeploymentData {
  repository: {
    deployments: {
      nodes: TestDeploymentNode[]
      pageInfo?: {
...
var octokit: Parameters<typeof latestActiveDeployment>[0]
var context: Parameters<typeof latestActiveDeployment>[1]
var mockDeploymentData: TestDeploymentData
var mockDeploymentResults: TestDeploymentNode
beforeEach(() => {
  vi.clearAllMocks()
  process.env.GITHUB_SERVER_URL = 'https://github.com'

  context = asPartialContext({
    repo: {
      owner: 'corp',
      repo: 'test'
    },
...
const createMockGraphQLOctokit = (data: unknown) =>
  asPartialOctokit({graphql: vi.fn().mockReturnValueOnce(data)})
