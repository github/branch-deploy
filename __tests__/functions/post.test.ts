...
beforeEach(() => {
  ...
  vi.spyOn(core, 'getBooleanInput').mockImplementation(name => {
    return validBooleanInputs[name as keyof typeof validBooleanInputs]
  })
  vi.spyOn(core, 'getInput').mockImplementation(name => {
    return validInputs[name as keyof typeof validInputs]
  })
  vi.spyOn(core, 'getState').mockImplementation(name => {
    return validStates[name as keyof typeof validStates]
  })

  asMock(vi.spyOn(postDeploy, 'postDeploy')).mockImplementation(() => {
    return undefined as unknown as Awaited<
      ReturnType<typeof postDeploy.postDeploy>
    >
  })
...
test('successfully runs post() Action logic when environment_url is not defined', async () => {
  ...
  vi.spyOn(core, 'getState').mockImplementation(name => {
    return noEnvironmentUrl[
      name as keyof typeof noEnvironmentUrl
...
test('exits due to a bypass being set', async () => {
  ...
  vi.spyOn(core, 'getState').mockImplementation(name => {
    return bypassed[name as keyof typeof bypassed]
  })
  expect(await post()).toBeUndefined()
...
test('skips the process of completing a deployment', async () => {
  ...
  vi.spyOn(core, 'getBooleanInput').mockImplementation(name => {
    return skipped[name as keyof typeof skipped] as unknown as boolean
  })
  expect(await post()).toBeUndefined()
  expect(infoMock).toHaveBeenCalledWith(
